  ...
  return (
    <XMenu>
      ...
      <XMenu.List
        ...
        preventScroll={false}
      >
        <ul className="flex flex-col h-full " data-testid="grok-menu-list">
          {options.map((opt, index) => (
            <li key={opt.value} className="flex-1">
              <button
                data-testid={`grok-option-${opt.value.toLowerCase().replace(/\s+/g, '-')}`}
                ...
                className={`h-full cursor-pointer w-full flex items-center gap-2 pl-2 outline-none ${
                  index === 0
                    ? ' hover:bg-white/10 focus:bg-white/10 rounded-t-2xl'
                    : canEnhanceTweet
                      ? ' rounded-b-2xl  hover:bg-white/10 focus:bg-white/10'
                      : ''
                }`}
              >
                <Icon
                  ...
                  disabled={true}
                />
                <span className="text-left text-sm font-bold text-text-active whitespace-nowrap">
                  {opt.value}
                </span>
              </button>
            </li>
